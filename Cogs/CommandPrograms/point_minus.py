...
class minus:
    async def minus_info(self, interaction, minus_point, user):
        await interaction.response.defer(ephemeral=False)

        # 指定された加算するポイントがプラスの値であるかどうか
        if minus_point > 0:
            embed=discord.Embed(title="エラー!", description="入力された減算値がマイナスではありません！\n入力した内容を確認してください。", color=0xff0000)
        ...
        if judge != 1:
            ...
            # あればそれに加算
                ...
                point = read_data[location]["point"] + minus_point
        ...
        # 結果をDiscordに表示
        embed=discord.Embed(title="成功！", description="<@" + str(user.id) + ">さんのポイントを" + str(minus_point) + "ポイント減算しました！", color=0x00ff7f)
